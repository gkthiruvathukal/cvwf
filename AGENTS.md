...
6. **`author+an` (per-author role annotation: self/graduate/undergrad) is passed through as a raw string (`authorAnnotations`) and not yet rendered.** Parsing it into structured per-author roles and highlighting them (bold self, italicize students) is explicitly deferred — don't build this speculatively; it's tracked in `TODO.md` for when it's requested.
